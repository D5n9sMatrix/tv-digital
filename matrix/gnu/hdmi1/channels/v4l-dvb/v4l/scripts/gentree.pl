...
GetOptions( "debug|d" => \$DEBUG,
	    "strip_dead_code|strip-dead-code|dead-code" => \$dead_code ) or die;
...
if (!defined($DESTDIR)) {
	print "Usage:\ngentree.pl\t[--debug] [--strip_dead_code] <version> <source dir> <dest dir>\n\n";
	exit;
}
...
#		next if ($line =~ m/[\$]Id:/);
		...
		elsif (!$dead_code && ($line =~ m@^\s*#\s*if\s*([01])[^\d]@ || $line =~ m@^\s*#\s*if\s*([01])$@)) {
			$state{$level} = "ifother";
			$if{$level} = 1;
